defmodule S2.FunctionalProg.Games.Hangman do
  defp seleccion() do
  ...
  defp juego(palJ, selPal, oport, dib_j) when palJ != selPal and oport < 6 do
    IO.puts(dib_j)
    IO.puts(palJ)
  ...
  defp juego(palJ, selPal, _ ,_) when palJ == selPal, do: {:guessed, selPal}

  defp juego(_, _, oport, dib_j) when oport == 6, do:  {:gameover, dib_j}

  defp actualizarPalabra(palJ, ingL, selPal) do
    mod = modificaciones(selPal, ingL)
    procesar(mod, length(mod), palJ)
  end

  defp modificaciones(selPal, ingL) do
    l_selPal = String.codepoints(selPal)
    Enum.filter(Enum.with_index(l_selPal), fn(t) -> elem(t, 0) == ingL end)
  end

  defp procesar(l_mods, tot, palJ) when tot > 0 do
  ...
  defp actualizarDibujo(oport, dib_j) when oport <= 6 do
    partes = ["O", "|", "/", "\\", "/", "\\"]
